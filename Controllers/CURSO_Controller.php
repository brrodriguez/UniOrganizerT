...
switch ($_REQUEST['accion']) { //Actúa según la acción elegida
    case 'vistainsertar':
        ...
        if (!tienePermisos('../templates/CursoAdd.html.twig')) {
            $template = $twig->loadTemplate('Mensaje.html.twig');
    		echo $template->render(array('strings' => $strings, 'respuesta' => 'No tienes los permisos necesarios', 'volver' => '../Controllers/CURSO_Controller.php'));
        } else {
			$idCalendario = ObtenerCalendario($_SESSION['login']);
			$template = $twig->loadTemplate('CursoAdd.html.twig');
            echo $template->render(array('strings' => $strings, 'idCalendario' => $idCalendario, 'volver' => '../Controllers/CURSO_Controller.php'));
        }
        break;
		
	case 'vistafiltrar':
      
        if (!tienePermisos('../templates/CursoFilter.html.twig')) {
			$template = $twig->loadTemplate('Mensaje.html.twig');
    		echo $template->render(array('strings' => $strings, 'respuesta' => 'No tienes los permisos necesarios', 'volver' => '../Controllers/CURSO_Controller.php'));
        } else {
			$template = $twig->loadTemplate('CursoFilter.html.twig');
            echo $template->render(array('strings' => $strings, 'volver' => '../Controllers/CURSO_Controller.php'));
        }
        break;
	...
	case 'filtrar':
        ...
        if (!tienePermisos('../templates/CursoFilter.html.twig')) {
			...
			$template = $twig->loadTemplate('CursoShowall.html.twig');
            echo $template->render(array('strings' => $strings, 'datos' => $datos, 'volver' => '../Controllers/CURSO_Controller.php'));
        }
        break;
		
	case 'vistaimportar':
      
        if (!tienePermisos('../templates/CursoImport.html.twig')) {
            $template = $twig->loadTemplate('Mensaje.html.twig');
    		echo $template->render(array('strings' => $strings, 'respuesta' => 'No tienes los permisos necesarios', 'volver' => '../Controllers/CURSO_Controller.php'));
        } else {
			...
			$template = $twig->loadTemplate('CursoImport.html.twig');
            echo $template->render(array('strings' => $strings, 'idCalendario' => $idCalendario, 'asignaturas' => $asignaturas, 'seleccionadas' => $seleccionadas, 'volver' => '../Controllers/CURSO_Controller.php'));
        }
        break;	
	...
	case 'importar':
        ...
        if (!tienePermisos('../templates/CursoImport.html.twig')) {
			...
			if($_REQUEST['curso'] === "0") {	
				$template = $twig->loadTemplate('Mensaje.html.twig');
    			echo $template->render(array('strings' => $strings, 'respuesta' => 'No se importó correctamente porque no se seleccionó ningún curso', 'volver' => '../Controllers/CURSO_Controller.php'));
			}else{
				if($_REQUEST['curso'] === "1") {
					$curso = get_data_form();
			...
			$template = $twig->loadTemplate('Mensaje.html.twig');
    		echo $template->render(array('strings' => $strings, 'respuesta' => $respuesta, 'volver' => '../Controllers/CURSO_Controller.php'));
        }
        break;	

    case 'ver':
        
        if (!tienePermisos('../templates/CursoShow.html.twig')) {
			$template = $twig->loadTemplate('Mensaje.html.twig');
    		echo $template->render(array('strings' => $strings, 'respuesta' => 'No tienes los permisos necesarios', 'volver' => '../Controllers/CURSO_Controller.php'));
        } else {
            $curso = get_data_form();
			...
			$template = $twig->loadTemplate('CursoShow.html.twig');
            echo $template->render(array('strings' => $strings, 'datos' => $datos, 'volver' => '../Controllers/CURSO_Controller.php'));
        }
        break;
	...
	case 'vistadesasignar':
        ...
        if (!tienePermisos('../templates/CursoAssign.html.twig')) {
            $template = $twig->loadTemplate('Mensaje.html.twig');
    		echo $template->render(array('strings' => $strings, 'respuesta' => 'No tienes los permisos necesarios', 'volver' => '../Controllers/CURSO_Controller.php'));
        } else {
            $curso = get_data_form();
            $datos['curso'] = $curso->obtenerCursoDetalle($_REQUEST['id']);
            $datos['asignaturas'] = $curso->obtenerRelacion_CursoAsignaturas($_REQUEST['id']);
			$template = $twig->loadTemplate('CursoUnassign.html.twig');
            echo $template->render(array('strings' => $strings, 'datos' => $datos, 'volver' => '../Controllers/CURSO_Controller.php'));
        }
        break;
    ...
    case 'vistaasignar':
        ...
        if (!tienePermisos('../templates/CursoAssign.html.twig')) {
			...
			$template = $twig->loadTemplate('CursoAssign.html.twig');
            echo $template->render(array('strings' => $strings, 'datos' => $datos, 'otros' => $otros, 'volver' => '../Controllers/CURSO_Controller.php'));
		}
        break;
	...
	case 'desasignar':
        ...
        if (!tienePermisos('../templates/CursoUnassing.html.twig')) {
            $template = $twig->loadTemplate('Mensaje.html.twig');
    		echo $template->render(array('strings' => $strings, 'respuesta' => 'No tienes los permisos necesarios', 'volver' => '../Controllers/CURSO_Controller.php'));
        } else {
            $curso = get_data_form();
    ...
    case 'vistamodificar':
        ...
        if (!tienePermisos('../templates/CursoEdit.html.twig')) {
            $template = $twig->loadTemplate('Mensaje.html.twig');
    		echo $template->render(array('strings' => $strings, 'respuesta' => 'No tienes los permisos necesarios', 'volver' => '../Controllers/CURSO_Controller.php'));
        } else {
            $curso = get_data_form();
			$datos = $curso->obtenerCursoDetalle($_REQUEST['id']);
			$template = $twig->loadTemplate('CursoEdit.html.twig');
            echo $template->render(array('strings' => $strings, 'datos' => $datos, 'volver' => '../Controllers/CURSO_Controller.php'));
        }
        break;
	...
	case 'vistaeliminar':
        ...
        if (!tienePermisos('../templates/CursoDelete.html.twig')) {
            $template = $twig->loadTemplate('Mensaje.html.twig');
    		echo $template->render(array('strings' => $strings, 'respuesta' => 'No tienes los permisos necesarios', 'volver' => '../Controllers/CURSO_Controller.php'));
        } else {
            $curso = get_data_form();
			$datos = $curso->obtenerCursoDetalle($_REQUEST['id']);
			$template = $twig->loadTemplate('CursoDelete.html.twig');
            echo $template->render(array('strings' => $strings, 'datos' => $datos, 'volver' => '../Controllers/CURSO_Controller.php'));
        }
        break;
